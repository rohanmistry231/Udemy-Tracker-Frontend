...
async function syncCoursesWithBackend() {
    try {
      ...
      backendCourses.forEach((backendCourse) => {
        const index = localCourses.findIndex((localCourse) => localCourse._id === backendCourse._id);
      ...
      localCourses = localCourses.filter((localCourse) =>
        backendCourses.some((backendCourse) => backendCourse._id === localCourse._id)
      );
...
export async function syncNotesWithBackend() {
    try {
      ...
      backendNotes.forEach((backendNote) => {
        const index = localNotes.findIndex((localNote) => localNote._id === backendNote._id);
